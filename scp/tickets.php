        ...
        case 'transfer': /** Transfer ticket **/
            ...
            if(!$thisstaff->canTransferTickets())
                ...
                if(!$errors && $ticket->transfer($_POST['deptId'], $_POST['transfer_comments'])) {
                    $msg = sprintf(_('Ticket transferred successfully to %s'),$ticket->getDeptName());
        ...
        case 'assign':
             ...
             if(!$thisstaff->canAssignTickets())
                 ...
                 if($claim && !$_POST['assign_comments'])
                     $_POST['assign_comments'] = sprintf(_('Ticket claimed by %s'),$thisstaff->getName());
                 ...
                 if(!$errors && $ticket->assign($_POST['assignId'], $_POST['assign_comments'], !$claim)) {
                     if($claim) {
                         ...
                         $msg=sprintf(_('Ticket assigned successfully to %s'), $ticket->getAssigned());
            ...
            switch(strtolower($_POST['do'])):
                case 'close':
                    if(!$thisstaff->canCloseTickets()) {
                        ...
                        $msg=sprintf(_('Ticket #%d status set to CLOSED'),$ticket->getExtId());
                        //Log internal note
                ...
                case 'release':
                    if(!$ticket->isAssigned() || !($assigned=$ticket->getAssigned())) {
                        ...
                        $msg=sprintf(_('Ticket released (unassigned) from %1$s by %2$s)'),$assigned,$thisstaff->getName());
                ...
                case 'claim':
                    if(!$thisstaff->canAssignTickets()) {
                        ...
                        $errors['err'] = sprintf(_('Ticket is already assigned to %s'),$ticket->getAssigned());
                    } elseif($ticket->assignToStaff($thisstaff->getId(), (sprintf(_('Ticket claimed by %s'),$thisstaff->getName())), false)) {
                        $msg = _('Ticket is now assigned to you!');
                    } else {
                ...
                case 'overdue':
                    ...
                    if(!$dept || !$dept->isManager($thisstaff)) {
                        ...
                        $msg=sprintf(_('Ticket flagged as overdue by %s'),$thisstaff->getName());
                        $ticket->logActivity(_('Ticket Marked Overdue'),$msg);
                    } else {
                ...
                case 'answered':
                    ...
                    if(!$dept || !$dept->isManager($thisstaff)) {
                        ...
                        $msg=sprintf(_('Ticket flagged as answered by %s'),$thisstaff->getName());
                        $ticket->logActivity(_('Ticket Marked Answered'),$msg);
                    } else {
                ...
                case 'unanswered':
                    ...
                    if(!$dept || !$dept->isManager($thisstaff)) {
                        ...
                        $msg=sprintf(_('Ticket flagged as unanswered by %s'),$thisstaff->getName());
                        $ticket->logActivity(_('Ticket Marked Unanswered'),$msg);
                    } else {
                ...
                case 'delete': // Dude what are you trying to hide? bad customer support??
                    if(!$thisstaff->canDeleteTickets()) {
                        ...
                        $msg=sprintf(_('Ticket #%d deleted successfully'),$ticket->getNumber());
                        //Log a debug note
                        $ost->logDebug(sprintf(_('Ticket #%d deleted'),$ticket->getNumber()),
                                sprintf(_('Ticket #%1$s deleted by %2$s'),
            ...
            case 'mass_process':
                ...
                elseif(!$_POST['tids'] || !is_array($_POST['tids']))
                    ...
                    switch(strtolower($_POST['do'])) {
                        case 'reopen':
                            if($thisstaff->canCloseTickets() || $thisstaff->canCreateTickets()) {
                                $note=sprintf(_('Ticket reopened by %s'),$thisstaff->getName());
                                ...
                                if($i==$count)
                                    $msg = sprintf(_('Selected tickets %d reopened successfully'),$i);
                                elseif($i)
                                    $warn = sprintf(_('%1$d of %2$d selected tickets reopened'),$i, $count);
                                else
                        ...
                        case 'close':
                            if($thisstaff->canCloseTickets()) {
                                $note=sprintf(_('Ticket closed without response by %s'),$thisstaff->getName());
                                foreach($_POST['tids'] as $k=>$v) {
                                ...
                                if($i==$count)
                                    $msg =sprintf(_('Selected tickets (%d) closed successfully'),$i);
                                elseif($i)
                                    $warn = sprintf(_('%1$d of %2$d selected tickets closed'),$i, $count);
                                else
                            ...
                            break;
                        case 'mark_overdue':
                            $note=sprintf(_('Ticket flagged as overdue by %s'),$thisstaff->getName());
                            foreach($_POST['tids'] as $k=>$v) {
                            ...
                            if($i==$count)
                                $msg = sprintf(_('Selected tickets (%d) marked overdue'), $i);
                            elseif($i)
                                $warn = sprintf(_('%1$d of %2$d selected tickets marked overdue'), $i, $count);
                            else
                        ...
                        case 'delete':
                            if($thisstaff->canDeleteTickets()) {
                                ...
                                //Log a warning
                                if($i) {
                                    $log = sprintf(_('%1$s (%2$s) just deleted %3$d ticket(s)'),
                                ...
                                if($i==$count)
                                    $msg = sprintf(_('Selected tickets (%d) deleted successfully'),$i);
                                elseif($i)
                                    $warn = sprintf(_('%1$d of %2$d selected tickets deleted'),$i, $count);
                                else
...
if($cfg->showAnsweredTickets()) {
    $nav->addSubMenu(array('desc'=>_('Open').' ('.number_format($stats['open']+$stats['answered']).')',
    ...
    if($stats) {
        $nav->addSubMenu(array('desc'=>_('Open').' ('.number_format($stats['open']).')',
                               'title'=>_('Open Tickets'),
                               'href'=>'tickets.php',
                               'iconclass'=>'Ticket'),
                            (!$_REQUEST['status'] || $_REQUEST['status']=='open'));
    ...
    if($stats['answered']) {
        $nav->addSubMenu(array('desc'=>_('Answered').' ('.number_format($stats['answered']).')',
...
if($stats['assigned']) {
    if(!$ost->getWarning() && $stats['assigned']>10)
        $ost->setWarning(sprintf(_('%d tickets assigned to you! Do something about it!'),$stats['assigned']));

    $nav->addSubMenu(array('desc'=>_('My Tickets').' ('.number_format($stats['assigned']).')',
...
if($stats['overdue']) {
    $nav->addSubMenu(array('desc'=>_('Overdue').' ('.number_format($stats['overdue']).')',
    ...
    if(!$sysnotice && $stats['overdue']>10)
        $sysnotice=sprintf(_('%d overdue tickets!'),$stats['overdue']);
}

if($thisstaff->showAssignedOnly() && $stats['closed']) {
    $nav->addSubMenu(array('desc'=>_('My Closed Tickets').' ('.number_format($stats['closed']).')',
    ...
    $nav->addSubMenu(array('desc'=>_('Closed Tickets').' ('.number_format($stats['closed']).')',
...
$inc = 'tickets.inc.php';
if($ticket) {
    $ost->setPageTitle(sprintf(_('Ticket #%d'),$ticket->getNumber()));
