...
if($_POST && $_POST['id']!=$thisstaff->getId()) { //Check dummy ID used on the form.
 $errors['err']=_('Internal Error. Action Denied');
    ...
    elseif($staff->updateProfile($_POST,$errors)){
        ...
        $errors['err']=_('Profile update error. Try correcting the errors below and try again!');
}

//Forced password Change.
if($thisstaff->forcePasswdChange() && !$errors['err'])
    $errors['err']=sprintf(_('<b>Hi %s</b> - You must change your password to continue!'),$thisstaff->getFirstName());
elseif($thisstaff->onVacation() && !$warn)
    $warn=sprintf(_("<b>Welcome back %s</b>! You are listed as 'on vacation' Please let your manager know that you are back."),$thisstaff->getFirstName());
