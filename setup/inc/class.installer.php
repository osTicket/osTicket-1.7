    ...
    function install($vars) {
        ...
        if(!$this->errors) {
            if(!db_connect($vars['dbhost'],$vars['dbuser'],$vars['dbpass']))
                $this->errors['db']=__('Unable to connect to MySQL server. Possibly invalid login info.');
            elseif(db_version()< $this->getMySQLVersion())
                $this->errors['db']=sprintf(__('osTicket requires MySQL %s or better!'),$this->getMySQLVersion());
        ...
        if(!file_exists($schemaFile))
            $this->errors['err']=__('Internal Error - please make sure your download is the latest (#1)');
        elseif(!($signature=trim(file_get_contents("$schemaFile.md5"))) || strcasecmp($signature, md5_file($schemaFile)))
            $this->errors['err']=sprintf(__('Unknown or invalid schema signature (%1$s .. %2$s)'), $signature, md5_file($schemaFile));
        ...
        elseif(!$this->load_sql_file($schemaFile,$vars['prefix'], true, $debug))
            $this->errors['err']=__('Error parsing SQL schema! Get help from developers (#4)');
        ...
        if(mysql_query($sql) && ($tid=mysql_insert_id())) {
            if(!($msg=file_get_contents(INC_DIR.'msg/installed.txt')))
                $msg=__('Congratulations and Thank you for choosing osTicket!');
