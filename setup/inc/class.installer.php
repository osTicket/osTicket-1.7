...
class Installer extends SetupWizard {
    ...
    function install($vars) {
        ...
        $f['dbhost']        = array('type'=>'string',   'required'=>1, 'error'=>_('Host name required'));
        ...
        if(!$this->errors && strcasecmp($vars['passwd'],$vars['passwd2']))
            $this->errors['passwd2']=_('Password(s) do not match');
        ...
        if(!$this->errors) {
            ...
            elseif(!db_select_database($vars['dbname']) && !db_create_database($vars['dbname'])) {
                ...
                if(mysql_query($sql)) {
                    $this->errors['err'] = _('We have a problem - another installation with same table prefix exists!');
                    $this->errors['prefix'] = _('Prefix already in-use');
                } else {
        ...
        elseif(!($signature=trim(file_get_contents("$schemaFile.md5"))) || strcasecmp($signature, md5_file($schemaFile)))
            $this->errors['err']=sprintf(_('Unknown or invalid schema signature (%1$s .. %2$s)'), $signature, md5_file($schemaFile));
        ...
        if(!$this->errors) {
            ...
            if(!mysql_query($sql) || !($cid=mysql_insert_id()))
                $this->errors['err']=_('Unable to create config settings (#7)');
        }
