...
class ApiController {
    ...
    function requireApiKey() {
        ...
        if(!($key=$this->getApiKey()))
            return $this->exerr(401, _('Valid API key required'));
        elseif (!$key->isActive() || $key->getIPAddr()!=$_SERVER['REMOTE_ADDR'])
            return $this->exerr(401, _('API key not found/active or source IP not authorized'));
    ...
    function getRequest($format) {
        ...
        if (!($stream = @fopen($input, 'r')))
            $this->exerr(400, _("Unable to read request body"));

        $parser = null;
        switch(strtolower($format)) {
            case 'xml':
                if (!function_exists('xml_parser_create'))
                    $this->exerr(501, _('XML extension not supported'));
            ...
            case 'email':
                ...
                break;
            default:
                $this->exerr(415, _('Unsupported data format'));
        }
    ...
    function validateRequestStructure($data, $structure, $prefix="") {
        ...
        foreach ($data as $key=>$info) {
            ...
            return $this->exerr(400, "$prefix$key: "._("Unexpected data received"));
        }
    ...
    function exerr($code, $error='') {
        ...
        $ost->logWarning(_('API Error')." ($code)", $msg, false);
