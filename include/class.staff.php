...
class Staff {
	...
	/*checks either ldap or internal passwd*/
	function check_ldap_passwd($username, $password)
	{
		$ret=false;
		if(LDAP::ldapActive())
		{
			$ret=LDAP::ldapAuthenticate($username,$password);
			if($ret==false)
			{
				$ret=$this->check_passwd($password);
			}
		}
		else
		{
    ...
    function login($username, $passwd, &$errors, $strike=true) {
        ...
        if(($user=new StaffSession(trim($username))) && $user->getId() && $user->check_ldap_passwd($username,$passwd)==true) {
