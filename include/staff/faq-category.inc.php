...
<div>
    ...
    <span>(<?php echo $category->isPublic()?_('Public'):_('Internal'); ?>)</span>
    <time> <?php echo _('Last updated').' '. Format::db_daydatetime($category->getUpdateDate()); ?></time>
</div>
<div class="cat-desc">
...
if($thisstaff->canManageFAQ()) {
    echo sprintf('<div class="cat-manage-bar"><a href="categories.php?id=%d" class="Icon editCategory">'._('Edit Category').'</a>
             <a href="categories.php" class="Icon deleteCategory">'._('Delete Category').'</a>
             <a href="faq.php?cid=%d&a=add" class="Icon newFAQ">'._('Add New FAQ').'</a></div>',
            $category->getId(),
            $category->getId());
} else {
?>
