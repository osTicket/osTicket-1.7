<?php
if(!defined('OSTSTAFFINC') || !$category || !$thisstaff) die(lang('access_denied'));
...
<div>
    ...
    <span>(<?php echo $category->isPublic()?lang('public'):lang('internal'); ?>)</span>
    <time><?php echo lang('last_updated'); ?> <?php echo Format::db_daydatetime($category->getUpdateDate()); ?></time>
</div>
<div class="cat-desc">
...
if($thisstaff->canManageFAQ()) {
    echo sprintf('<div class="cat-manage-bar"><a href="categories.php?id=%d" class="Icon editCategory">'.lang('edit_category').'</a>
             <a href="categories.php" class="Icon deleteCategory">'.lang('delete_category').'</a>
             <a href="faq.php?cid=%d&a=add" class="Icon newFAQ">'.lang('add_new_faq').'</a></div>',
            $category->getId(),
            $category->getId());
} else {
?>
...
if(($res=db_query($sql)) && db_num_rows($res)) {
    ...
    echo '<strong>'.lang('catg_no_have_faqs').'</strong>';
}
?>
