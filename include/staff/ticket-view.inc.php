...
if($ticket->isAssigned() && (
    ...
    $warn.='&nbsp;&nbsp;<span class="Icon assignedTicket">'.sprintf(_('Ticket is assigned to %s'),implode('/', $ticket->getAssignees())).'</span>';
if(!$errors['err'] && ($lock && $lock->getStaffId()!=$thisstaff->getId()))
    $errors['err']=sprintf(_('This ticket is currently locked by %s'),$lock->getStaffName());
...
<table width="940" cellpadding="2" cellspacing="0" border="0">
    <tr>
        <td width="50%" class="has_bottom_border">
             <h2><a href="tickets.php?id=<?php echo $ticket->getId(); ?>" title="Reload"><i class="icon-refresh"></i> <?php echo sprintf(_('Ticket #%d'), $ticket->getExtId()); ?></a></h2>
        </td>
        <td width="50%" class="right_align has_bottom_border">
            <?php
            if($thisstaff->canBanEmails() || ($dept && $dept->isManager($thisstaff))) { ?>
            <span class="action-button" data-dropdown="#action-dropdown-more">
                <span ><i class="icon-cog"></i> <?php echo _('More');?></span>
                <i class="icon-caret-down"></i>
            </span>
            <?php
            } ?>
            <?php if($thisstaff->canDeleteTickets()) { ?>
                <a id="ticket-delete" class="action-button" href="#delete"><i class="icon-trash"></i> <?php echo _('Delete');?></a>
            <?php } ?>
            <?php 
            if($thisstaff->canCloseTickets()) {
                if($ticket->isOpen()) {?>
                <a id="ticket-close" class="action-button" href="#close"><i class="icon-remove-circle"></i> <?php echo _('Close');?></a>
                <?php
                } else { ?>
                <a id="ticket-reopen" class="action-button" href="#reopen"><i class="icon-undo"></i> <?php echo _('Reopen');?></a>
                <?php
                } ?>
            <?php 
            } ?>
            ...
            if($thisstaff->canEditTickets()) { ?>
                <a class="action-button" href="tickets.php?id=<?php echo $ticket->getId(); ?>&a=edit"><i class="icon-edit"></i> <?php echo _('Edit');?></a>
            <?php 
            } ?>

            <?php
            if($ticket->isOpen() && !$ticket->isAssigned() && $thisstaff->canAssignTickets()) {?>
                <a id="ticket-claim" class="action-button" href="#claim"><i class="icon-user"></i> <?php echo _('Claim');?></a>
                
            <?php
            }?>

            <a id="ticket-print" class="action-button" href="tickets.php?id=<?php echo $ticket->getId(); ?>&a=print"><i class="icon-print"></i> <?php echo _('Print');?></a>
            ...
            <div id="action-dropdown-more" class="action-dropdown anchor-right">
              <ul>
                ...
                if($ticket->isOpen() && ($dept && $dept->isManager($thisstaff))) {
                    ...
                    if($ticket->isAssigned()) { ?>
                        <li><a id="ticket-release" href="#release"><i class="icon-user"></i> <?php echo _('Release (unassign) Ticket');?></a></li>
                    <?php
                    }
                    
                    if(!$ticket->isOverdue()) { ?>
                        <li><a id="ticket-overdue" href="#overdue"><i class="icon-bell"></i> <?php echo _('Mark as Overdue');?></a></li>
                    <?php
                    }
                    
                    if($ticket->isAnswered()) { ?>
                        <li><a id="ticket-unanswered" href="#unanswered"><i class="icon-circle-arrow-left"></i> <?php echo _('Mark as Unanswered');?></a></li>
                    <?php
                    } else { ?>
                        <li><a id="ticket-answered" href="#answered"><i class="icon-circle-arrow-right"></i> <?php echo _('Mark as Answered');?></a></li>
                    <?php
                    }
                }
                ...
                if($thisstaff->canBanEmails()) { 
                     if(!$emailBanned) {?>
                        <li><a id="ticket-banemail" href="#banemail"><i class="icon-ban-circle"></i> <?php echo _('Ban Email');?> (<?php echo $ticket->getEmail(); ?>)</a></li>
                <?php 
                     } elseif($unbannable) { ?>
                        <li><a id="ticket-banemail" href="#unbanemail"><i class="icon-undo"></i> <?php echo _('Unban Email');?> (<?php echo $ticket->getEmail(); ?>)</a></li>
                    <?php
                     }
                }?>
              </ul>
                            ...
                            <div id="action-dropdown-stats" class="action-dropdown anchor-right">
                                <ul>
                                    ...
                                    if(($open=$client->getNumOpenTickets()))
                                        echo sprintf('<li><a href="tickets.php?a=search&status=open&query=%s"><i class="icon-folder-open-alt"></i> '._('%d Open Tickets').'</a></li>',
                                                $ticket->getEmail(), $open);
                                    if(($closed=$client->getNumClosedTickets()))
                                        echo sprintf('<li><a href="tickets.php?a=search&status=closed&query=%s"><i class="icon-folder-close-alt"></i> '._('%d Closed Tickets').'</a></li>',
                                                $ticket->getEmail(), $closed);
                                    ?>
                                    <li><a href="tickets.php?a=search&query=<?php echo $ticket->getEmail(); ?>"><i class="icon-double-angle-right"></i> <?php echo _('All Tickets');?></a></li>
                                </u>
                            </div>
                    <?php
                        }
    ...
    <tr>
        <td width="50%">
            <table cellspacing="0" cellpadding="4" width="100%" border="0">
                ...
                if($ticket->isOpen()) { ?>
                <tr>
                    <th width="100"><?php echo _('Assigned To');?>:</th>
                    <td>
                        <?php
                        if($ticket->isAssigned())
                ...
                <tr>
                    <th width="100"><?php echo _('Closed By');?>:</th>
                    <td>
                        <?php
        ...
        <td width="50%">
            <table cellspacing="0" cellpadding="4" width="100%" border="0">
                <tr>
                    <th width="100"><?php echo _('Help Topic');?>:</th>
                    <td><?php echo Format::htmlchars($ticket->getHelpTopic()); ?></td>
                </tr>
                <tr>
                    <th nowrap><?php echo _('Last Message');?>:</th>
                    <td><?php echo Format::db_datetime($ticket->getLastMsgDate()); ?></td>
                </tr>
                <tr>
                    <th nowrap><?php echo _('Last Response');?>:</th>
...
?>
<ul id="threads">
    <li><a class="active" id="toggle_ticket_thread" href="#"><?php echo _('Ticket Thread');?> (<?php echo $tcount; ?>)</a></li>
    <?php
    if(!$cfg->showNotesInline()) {?>
    <li><a id="toggle_notes" href="#"><?php echo _('Internal Notes');?> (<?php echo $ticket->getNumNotes(); ?>)</a></li>
    <?php
    }?>
</ul>
<?php
...
<div id="response_options">
    <ul>
        ...
        <li><a id="reply_tab" href="#reply"><?php echo _('Post Reply');?></a></li>
        <?php
        } ?>
        <li><a id="note_tab" href="#note"><?php echo _('Post Internal Note');?></a></li>
        <?php
        if($thisstaff->canTransferTickets()) { ?>
        <li><a id="transfer_tab" href="#transfer"><?php echo _('Dept. Transfer');?></a></li>
        <?php
        }
    ...
    <form id="reply" action="tickets.php?id=<?php echo $ticket->getId(); ?>#reply" name="reply" method="post" enctype="multipart/form-data">
        ...
        <table border="0" cellspacing="0" cellpadding="3">
            <tr>
                <td width="160">
                    <label><strong><?php echo _('TO');?>:</strong></label>
                </td>
                <td width="765">
                    <?php
                    $to = $ticket->getEmail();
                    ...
                    <label><input type='checkbox' value='1' name="emailreply" id="remailreply"
                        <?php echo ((!$info['emailreply'] && !$errors) || isset($info['emailreply']))?'checked="checked"':''; ?>> <?php echo _('Email Reply');?></label>
                </td>
            </tr>
            <?php
            if($errors['response']) {?>
            ...
            <tr>
                <td width="160">
                    <label><strong><?php echo _('Response');?>:</strong></label>
                </td>
                <td width="765">
                    <?php
                    if(($cannedResponses=Canned::responsesByDeptId($ticket->getDeptId()))) {?>
                        <select id="cannedResp" name="cannedResp">
                            <option value="0" selected="selected"><?php echo _('Select a canned response');?></option>
                            <?php
            ...
            <tr>
                <td width="160">
                    <label for="signature" class="left"><?php echo _('Signature');?>:</label>
                </td>
                <td width="765">
                    <?php
            ...
            <tr>
                <td width="160">
                    <label><strong><?php echo _('Ticket Status');?>:</strong></label>
                </td>
                <td width="765">
                    <?php
        ...
        <p  style="padding-left:165px;">
            <input class="btn_sm" type="submit" value="<?php echo _('Post Reply');?>">
            <input class="btn_sm" type="reset" value="<?php echo _('Reset');?>">
        </p>
    </form>
    <?php
    } ?>
    <form id="note" action="tickets.php?id=<?php echo $ticket->getId(); ?>#note" name="note" method="post" enctype="multipart/form-data">
        ...
        <table border="0" cellspacing="0" cellpadding="3">
            ...
            <tr>
                <td width="160">
                    <label><strong><?php echo _('Internal Note');?>:</strong></label>
                </td>
                <td width="765">
                    <div><span class="faded"><?php echo _('Note details');?></span>&nbsp;
                    ...
                    <div>
                        <span class="faded"><?php echo _('Note title');?> - <?php echo _('summarry of the note (optional)');?></span>&nbsp;
            ...
            <tr>
                <td width="160">
                    <label><?php echo _('Ticket Status');?>:</label>
                </td>
                <td width="765">
                    <div class="faded"></div>
                    <select name="state">
                        <option value="" selected="selected">&mdash; <?php echo _('unchanged');?> &mdash;</option>
                        <?php
                        $state = $info['state'];
                        if($ticket->isClosed()){ 
       ...
       <p  style="padding-left:165px;">
           <input class="btn_sm" type="submit" value="<?php echo _('Post Note');?>">
           <input class="btn_sm" type="reset" value="<?php echo _('Reset');?>">
       </p>
   </form>
    <?php
    ...
    <form id="transfer" action="tickets.php?id=<?php echo $ticket->getId(); ?>#transfer" name="transfer" method="post" enctype="multipart/form-data">
        ...
        <table border="0" cellspacing="0" cellpadding="3">
            ...
            <tr>
                <td width="160">
                    <label for="deptId"><strong><?php echo _('Department');?>:</strong></label>
                </td>
                <td width="765">
                    <?php
                    ...
                    <select id="deptId" name="deptId">
                        <option value="0" selected="selected">&mdash; <?php echo _('Select Target Department');?> &mdash;</option>
                        <?php
            ...
            <tr>
                <td width="160">
                    <label><strong><?php echo _('Comments');?>:</strong></label>
                </td>
                <td width="765">
                    <span class="faded"><?php echo _('Enter reasons for the transfer.');?></span>
        ...
        <p style="padding-left:165px;">
           <input class="btn_sm" type="submit" value="<?php echo _('Transfer');?>">
           <input class="btn_sm" type="reset" value="<?php echo _('Reset');?>">
        </p>
    </form>
    <?php
    ...
    <form id="assign" action="tickets.php?id=<?php echo $ticket->getId(); ?>#assign" name="assign" method="post" enctype="multipart/form-data">
        ...
        <table border="0" cellspacing="0" cellpadding="3">
            ...
            <tr>
                <td width="160">
                    <label for="assignId"><strong><?php echo _('Assignee');?>:</strong></label>
                </td>
                <td width="765">
                    <?php
            ...
            <tr>
                <td width="160">
                    <label><strong><?php echo _('Comments');?>:</strong><span class='error'>&nbsp;</span></label>
                </td>
                <td width="765">
                    <span class="faded"><?php echo _('Enter reasons for the assignment or instructions for assignee.');?></span>
...
<div style="display:none;" class="dialog" id="print-options">
    <h3><?php echo _('Ticket Print Options');?></h3>
    ...
    <form action="tickets.php?id=<?php echo $ticket->getId(); ?>" method="post" id="print-form" name="print-form">
        ...
        <fieldset class="notes">
            <label for="notes"><?php echo _('Print Notes');?>:</label>
            <input type="checkbox" id="notes" name="notes" value="1"> <?php echo _('Print <b>Internal</b> Notes/Comments');?>
        </fieldset>
        <fieldset>
            <label for="psize"><?php echo _('Paper Size');?>:</label>
            <select id="psize" name="psize">
                <option value="">&mdash; <?php echo _('Select Print Paper Size');?> &mdash;</option>
                <?php
        ...
        <p class="full-width">
            <span class="buttons" style="float:left">
                ...
                <input type="button" value="<?php echo _('Cancel');?>" class="close">
            </span>
            <span class="buttons" style="float:right">
                <input type="submit" value="<?php echo _('Print');?>">
            </span>
         </p>
    </form>
    <div class="clear"></div>
...
<div style="display:none;" class="dialog" id="ticket-status">
    <h3><?php echo sprintf(_('%1$s Ticket #%2$s'), ($ticket->isClosed()?_('Reopen'):_('Close')), $ticket->getNumber()); ?></h3>
    <a class="close" href="">&times;</a>
    <hr/>
    ...
    <form action="tickets.php?id=<?php echo $ticket->getId(); ?>" method="post" id="status-form" name="status-form">
        ...
        <fieldset>
            <em><?php echo _('Reasons for status change (internal note). Optional but highly recommended.');?></em><br>
...
<div style="display:none;" class="dialog" id="confirm-action">
    ...
    <p class="confirm-action" style="display:none;" id="claim-confirm">
        <?php echo _('Are you sure want to <b>claim</b> (self assign) this ticket?');?>
    </p>
    <p class="confirm-action" style="display:none;" id="answered-confirm">
        <?php echo _('Are you sure want to flag the ticket as <b>answered</b>?');?>
    </p>    
    <p class="confirm-action" style="display:none;" id="unanswered-confirm">
        <?php echo _('Are you sure want to flag the ticket as <b>unanswered</b>?');?>
    </p>
    <p class="confirm-action" style="display:none;" id="overdue-confirm">
        <?php echo _('Are you sure want to flag the ticket as <font color="red"><b>overdue</b></font>?');?>
    </p>
    <p class="confirm-action" style="display:none;" id="banemail-confirm">
        <?php echo sprintf(_('Are you sure want to <b>ban</b> %s?'), $ticket->getEmail());?> <br><br>
        <?php echo _('New tickets from the email address will be auto-rejected.');?>
    </p>
    <p class="confirm-action" style="display:none;" id="unbanemail-confirm">
        <?php echo sprintf(_('Are you sure want to <b>remove</b> %s from ban list?'), $ticket->getEmail()); ?> 
    </p>
    <p class="confirm-action" style="display:none;" id="release-confirm">
        <?php echo sprintf(_('Are you sure want to <b>unassign</b> ticket from %s<b>?'), $ticket->getAssigned()); ?></b>
    </p>
    <p class="confirm-action" style="display:none;" id="delete-confirm">
        <font color="red"><strong><?php echo _('Are you sure you want to DELETE this ticket?');?></strong></font>
        <br><br><?php echo _('Deleted tickets CANNOT be recovered, including any associated attachments.');?>
    </p>
