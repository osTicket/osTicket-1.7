...
class Dept {
    ...
    function save($id, $vars, &$errors) {
        ...
        if($id && $id!=$vars['id'])
            $errors['err']=lang('invalid_dept_id');
            
        if(!$vars['email_id'] || !is_numeric($vars['email_id']))
            $errors['email_id']=lang('email_selec_req');
            
        if(!is_numeric($vars['tpl_id']))
            $errors['tpl_id']=lang('template_sect_req');
        ...
        if(!$vars['name']) {
            ...
            $errors['name']=lang('dept_exist');
        }
        
        if(!$vars['ispublic'] && ($vars['id']==$cfg->getDefaultDeptId()))
            $errors['ispublic']=lang('def_dep_cant_priv');
        ...
        if($id) {
            ...
            $errors['err']=lang('cant_update').' '.Format::htmlchars($vars['name']).' '.lang('dept_error_ocur');
            ...
            $errors['err']=lang('cant_create_dept');
