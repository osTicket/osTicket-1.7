...
Class ThreadEntry {
    ...
    function uploadFiles($files) {
        ...
        foreach($files as $file) {
            ...
            if(!$file['error']
                ...
                if(!$file['error'])
                    $error = sprintf(_('Unable to upload file - %s'),$file['name']);
               ...
               */
                $this->getTicket()->logNote(_('File Upload Error'), $error, 'SYSTEM', false);
            }
    ...
    function importAttachment($attachment) {
        ...
        if (!$attachment['error'] && ($id=$this->saveAttachment($attachment)))
            ...
            if(!$error)
                $error = sprintf(_('Unable to import attachment - %s'),$attachment['name']);

            $this->getTicket()->logNote(_('File Import Error'), $error, 'SYSTEM', false);
        }
...
class Message extends ThreadEntry {
    ...
    function add($vars, &$errors) {
        ...
        if(!$vars || !is_array($vars) || !$vars['ticketId'])
            $errors['err'] = _('Missing or invalid data');
        elseif(!$vars['message'])
