...
class Category {
    ...
    function save($id, $vars, &$errors, $validation=false) {
        ...
        if(!$vars['name'])
            $errors['name']=_('Category name is required');
        elseif(strlen($vars['name'])<3)
            $errors['name']=_('Name is too short. 3 chars minimum');
        elseif(($cid=self::findIdByName($vars['name'])) && $cid!=$id)
            $errors['name']=_('Category already exists');

        if(!$vars['description'])
            $errors['description']=_('Category description is required');
        ...
        if($id) {
            ...
            $errors['err']=_('Unable to update FAQ category.');
            ...
            $errors['err']=_('Unable to create FAQ category. Internal error');
        }
